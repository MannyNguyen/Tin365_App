...
public class MediaController extends FrameLayout {
	...
	private RelativeLayout layoutTopBar,setNextPre;
	...
	private Runnable mRunnale = new Runnable() {
		@Override
		public void run() {
			if (mInstantSeeking) {
	...
	private OnSeekBarChangeListener mSeekListener = new OnSeekBarChangeListener() {
		public void onStartTrackingTouch(SeekBar bar) {
			System.out.println("MediaController, onStartTrackingTouch");
			mDragging = true;
			show(3600000);
		...
		public void onProgressChanged(SeekBar bar, int progress,
				boolean fromuser) {
			System.out.println("MediaController, onProgressChanged");
			if (!fromuser)
				return;
			_checkendtime = progress;
			long newposition = (mDuration * (progress-(5/2))) / 1000;
			curPos = newposition;
			System.out.println("curPos = " + curPos);
			String time = StringUtils.generateTime(newposition);
			mControllHandler.removeCallbacks(mRunnale);
			mControllHandler.postDelayed(mRunnale, 500);
			if (mInfoView != null)
		...
		public void onStopTrackingTouch(SeekBar bar) {
			System.out.println("MediaController, onStopTrackingTouch");
			if (!mInstantSeeking)
	...
	public void setTopBar(RelativeLayout layoutTopBar) {
		this.layoutTopBar = layoutTopBar;
	}
	public void setNextPre(RelativeLayout setNextPre) {
		this.setNextPre = setNextPre;
	}
	...
	public void show(int timeout) {
		if (!mShowing && mAnchor != null && mAnchor.getWindowToken() != null) {
			...
			if (mFromXml) {
				...
				setNextPre.setVisibility(View.VISIBLE);
			} else {
		...
		if (mShowing) {
			try {
				...
				if (mFromXml) {
					...
					setNextPre.setVisibility(View.GONE);
				} else
					mWindow.dismiss();
	...
	public void showBufferingInfo() {
		mFileName.setText(mContext.getString(R.string.buffering) + " 0%");
		mFileName.setVisibility(View.VISIBLE);
	}

	public void hideBufferingInfo(int timeout) {
		...
		mControllHandler.removeCallbacks(mRunnable);
		mControllHandler.postDelayed(mRunnable, 1000);
	}
	...
	private long setProgress() {
		...
		// sometimes percent stopped at 98 or 99
		if (percent >= 98) {
			percent = 100;
		}
		...
		if (mFileName != null)
			mFileName.setText(mContext.getString(R.string.buffering) + " " + String.valueOf(percent) + "%");
		if (mEndTime != null)
			mEndTime.setText(StringUtils.generateTime(mDuration));
		// set time when running
		if(check == 0)
		{
		if (mCurrentTime != null)
		...
		String start = StringUtils.generateTime(position);
		String end = StringUtils.generateTime(mDuration) ;
		boolean _check = start.equals(end);
		System.out.println("start = "+ start + "- end = "+end + " - _percent = " + _checkendtime);
		if(_check && _checkendtime == 1000)
		{
			mPlayer.start();
			check = 1;
			System.out.println("check = 1");
		}
		if(_checkendtime != 1000)
			check = 0;
		
		//fixed bug when drag 100%
		if(percent == 100)
		{
//			mPlayer.start();
//			updatePausePlay();
