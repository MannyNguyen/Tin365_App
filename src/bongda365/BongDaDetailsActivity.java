...
import com.skyward101.tin365.CommentActivity;
...
public class BongDaDetailsActivity extends SherlockFragmentActivity implements
	...
	private void initActionBar() {
		...
		final View v = inflator.inflate(R.layout.actionbar_bongda365_details,
				null);
		...
		((TextView) v.findViewById(R.id.tvCm)).setText(_cmt_count);
	...
	class asyTaskGetDetails extends AsyncTask<String, String, String> {
		...
		protected void onPostExecute(String result) {
			...
			// init day
			_day = Utility.getTime(details.getmDate(), BongDaDetailsActivity.this);
			// init day
			tvTitle.setText(details.getmTitle());
			...
			for (int i = 0; i < details.getmContent().size(); i++) {
				...
				// ///// add text
				if (!nb[0].equals("~")) {
					TextView tvv = new TextView(BongDaDetailsActivity.this);
				...
				for (int ti = 1; ti < nb.length; ti++) {
					...
					// ///// add text
					if (!nb[ti].equals("~")) {
						TextView tv1 = new TextView(BongDaDetailsActivity.this);
				...
				final ImageView img1 = new ImageView(BongDaDetailsActivity.this);
				...
				// .createDefault(BongDaDetailsActivity.this));
				// // Load and display image asynchronously
				...
				imageLoader.displayImage(details.getmImgUrls().get(i), img1,
						doption, new ImageLoadingListener() {
							...
							public void onLoadingComplete(String imageUri,
								...
								// .isPhone(BongDaDetailsActivity.this))
								// {
								try {
				...
				if (!details.getmImgHints().get(i).equals("~")) {
					TextView tvHint = new TextView(BongDaDetailsActivity.this);
	...
	private void addVideoAndTable() {
		...
		for (int i = 0; i < details.getmVideoUrls().size(); i++) {
			...
			if (details.getmVideoUrls().get(i).getVideos().size() > 1) {
				TextView tv2 = new TextView(BongDaDetailsActivity.this);
	...
	public void onClick(View v) {
		...
		case R.id.img_cat:
			...
			Intent i = new Intent(BongDaDetailsActivity.this,
	...
	private void getProfileExample() {
		...
		final OnProfileRequestListener onProfileRequestListener = new SimpleFacebook.OnProfileRequestListener() {
			...
			public void onComplete(Profile profile) {
				...
				SharePreferance share = new SharePreferance(
						BongDaDetailsActivity.this);
